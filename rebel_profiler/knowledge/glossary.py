...
GLOSSARY: tuple[tuple[str, str], ...] = (
    ...
    ("reverse engineering", "Reading a binary to understand what it does. Static RE treats the file as data; nothing is executed."),
    ("static analysis", "Analysis of a program without running it: headers, strings, symbols, disassembly. Safe by construction."),
    ("disassembly", "Decoding machine-code bytes into instructions for reading. The disassembler reads; it never executes."),
    ("nx bit", "No-execute page flag: data pages cannot run code. Its absence in a shipped binary is a mitigation gap."),
    ("pie", "Position-independent executable: loads at a randomized address. Compiling with PIE defeats fixed-address exploits."),
    ("stack canary", "A sentinel value checked before return; overwriting it aborts. Stack-smashing is detected, not silent."),
    ("relro", "Relocation read-only: GOT/memory tables made read-only after loading. Full RELRO closes GOT-overwrite paths."),
    ("checksec", "A read of a binary's compiled-in exploit mitigations (NX/PIE/canary/RELRO). Offline and analysis-only."),
    ("ioc candidate", "An artifact (string, address, import) that MIGHT indicate malicious behavior. Verification precedes any verdict."),
    ("c2", "Command and control: the channel malware uses for instructions. Embedded C2 endpoints are prime static findings."),
    ("detonation", "Running a sample to observe behavior — isolated external sandbox infrastructure only, never the workstation."),
    ("chain of custody", "Documented handling of evidence from acquisition to presentation: who held it, when, and its hash."),
