...
DEFAULT_SOURCES: tuple[Source, ...] = (
    ...
    Source("scan.webauth", "scan", "a1", 1.0, "authenticated session audit", "in-process one-login posture audit using an operator-stored test account"),
    Source("scan.proxy", "scan", "a1", 1.0, "loopback traffic recording", "requests the OPERATOR made to their own lab app, recorded on 127.0.0.1"),
    Source("scan.iac", "audit", "a1", 1.0, "IaC static audit", "trivy config scan of a file already on the operator's disk (offline)"),
