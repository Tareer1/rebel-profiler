...
CLI_WHITELIST = {
    ...
    # --- RE plane: offline reads of a possessed sample (target = path) ---
    "binary_info":     ["intel", "collect", "{case}", "binary-info", "{target}", "-y"],
    "checksec":        ["intel", "collect", "{case}", "checksec", "{target}", "-y"],
    "string_dump":     ["intel", "collect", "{case}", "string-dump", "{target}",
                        "-p", "max_lines", "{text}", "-y"],
    "symbol_dump":     ["intel", "collect", "{case}", "symbol-dump", "{target}", "-y"],
    "disasm":          ["intel", "collect", "{case}", "disasm", "{target}",
                        "-p", "section", "{text}", "-y"],
    # --- web-expansion adapters: CORS, security.txt, GraphQL, SPF/DMARC ---
    "cors_check":      ["intel", "collect", "{case}", "cors-check", "{target}",
                        "-p", "port", "{text}", "-p", "scheme", "{text2}", "-y"],
    "security_txt":    ["intel", "collect", "{case}", "security-txt", "{target}", "-y"],
    "graphql_introspection": ["intel", "collect", "{case}", "graphql-introspection",
                              "{target}", "-p", "path", "{text}", "-y"],
    "email_spoof":     ["intel", "collect", "{case}", "email-spoof", "{target}", "-y"],
    "vuln_coverage":   ["intel", "vuln-coverage", "{case}"],
    "cwe_lookup":      ["intel", "cwe", "lookup", "{text}"],
    "cwe_search":      ["intel", "cwe", "search", "{text}"],
    "cwe_blind_spots": ["intel", "cwe", "blind-spots", "{case}"],
    "anomalies":       ["intel", "anomalies", "{case}"],
